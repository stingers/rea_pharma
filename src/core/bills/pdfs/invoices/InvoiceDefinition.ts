import { getNbProductsOnSale, getTotalAmountAllIncludedOnSale, getTotalQtityOnSale, inSteGrp, labelSteName, Sale } from "asv-hlps";
import dayjs from "dayjs";

import { ConvertTo } from "../../../../shared/helpers/ConvertTo";
import { InvoiceAmount } from "./InvoiceAmount";

// import { Sale } from '../../shared/ConvertTo';

export class InvoiceDefinition {
  static invoice(tob: Sale, pdfTables: any) {
    // const steGrpName = valToSingular(tob.client.ste.grp.name );
    const clientSte = tob.client.ste ? labelSteName(tob.client.ste, 30) : "";
    const clientLastname = tob.client.lastname ? tob.client.lastname : "";
    const clientFirstname = tob.client.firstname ? tob.client.firstname : "";
    const clientGender = tob.client.gender ? tob.client.gender.abr : "";
    // const billRef =  (tob.bill) ? 'N° ' + tob.bill.ref : '/BL';
    let billRef;
    let waterMark;
    if (tob.bill) {
      billRef = "N° " + tob.bill.ref;
    } else if (tob.isProforma) {
      billRef = "/PROFORMA";
    } else if (tob.isBack) {
      billRef = "/RETOUR";
      if (tob.backChoice.toLowerCase() === "avoir") {
        waterMark = "retour accepté";
      }
      if (tob.backChoice.toLowerCase() === "autre") {
        waterMark = "le(s) produit(s) remplacé(s)";
      }
      if (tob.isRejected) {
        waterMark = "Retour refusé";
      }
    } else {
      billRef = "/BL";
    }
    const client = tob.client.ste ? clientSte : clientGender + " " + clientLastname + " " + clientFirstname;
    const cp = tob.client.cp ? tob.client.cp : "";
    const city = tob.client.city ? tob.client.city : "";
    // const nif =  (tob.client.ste.nif) ? tob.client.ste.nif : '';
    const address = tob.client.address ? tob.client.address : "";
    const totalQtity = getTotalQtityOnSale(tob, true);
    const nbProducts = getNbProductsOnSale(tob);
    const nbColis = tob.nbPackages ? tob.nbPackages : "---";
    const nbFreeze = tob.nbFreeze ? tob.nbFreeze : "---";
    const oWidth = ["*", "*", 75, 75, 75, 75];

    let infoAmount;
    if (inSteGrp(["ph"], tob.client) && !tob.client.isBadPayer) {
      infoAmount = InvoiceAmount.infoBl(tob);
    }
    if (tob.isProforma) {
      infoAmount = InvoiceAmount.infoProforma(tob);
    } else {
      infoAmount = InvoiceAmount.info(tob);
    }

    const toLetter = new ConvertTo();
    let nbrToLetter;
    if (!tob.isProforma) {
      nbrToLetter = toLetter.numberToLetter(Math.abs(getTotalAmountAllIncludedOnSale(tob, true))).toUpperCase();
    } else {
      nbrToLetter = toLetter.numberToLetter(Math.abs(getTotalAmountAllIncludedOnSale(tob))).toUpperCase();
    }

    let nbrToLetterText = "Arrêter cette facture à la somme de " + nbrToLetter + " francs CFA.";
    if (tob.isProforma) {
      nbrToLetterText = "Arrêter ce proforma à la somme de " + nbrToLetter + " francs CFA.";
    }

    if (tob && tob.backChoice === "Avoir") {
      nbrToLetterText = "Arrêter cette facture de CREDIT en votre faveur à la somme de " + nbrToLetter + " francs CFA.";
    }
    if (tob && tob.backChoice === "Autre") {
      nbrToLetterText = "Le(s) produit(s) vous sera(seront) remplacés dans les brefs délai ";
    }

    if (tob && tob.isRejected) {
      nbrToLetterText = "Votre retour n'a pas été accepté. Motif: " + tob.rejectedReason;
    }

    return {
      pageOrientation: "portrait",
      // alignment: 'justify',
      pageSize: "A4",
      pageMargins: [20, 150, 20, 70],
      watermark: { text: waterMark, opacity: 0.1, bold: true, italics: false },

      header: [
        {
          margin: [20, 20, 20, 0],
          columns: [
            [
              {
                columns: [
                  {
                    image:
                      "data:image/png;base64,",
                    width: 40,
                    height: 40,
                    alignment: "left",
                    // margin: [20, 20, 20, 20]
                  },
                  {
                    text: ["COMPAGNIE \n ", "PHARMACEUTIQUE \n ", "AFRICAINE \n"],
                    width: "auto",
                    alignment: "left",
                    fontSize: 12,
                    bold: true,
                    margin: [-15, 0, 0, 0],
                  },
                ],
              },
              {
                text: [
                  { text: "01 BP 3701 514 AVENUE DES CALAIS\n", fontSize: 8 },
                  { text: "Tél: (228) 22 22 27 71/ 22 22 34 74\n", fontSize: 8 },
                  { text: "NIF: 10000175770 \n", bold: true, margin: [0, 0, 0, 2], fontSize: 8 },
                  // {text: 'fax: (228) 22 22 13 68 LOME - TOGO\n', fontSize: 8}
                  { text: "AUTORISATION N°33/97/MS/DGS/DPET \n", bold: true, fontSize: 10 /* margin: [20, 5, 0, 0]  */ },
                ],
              },
            ],

            {
              qr: tob.ref,
              alignment: "right",
              fit: "50",
            },
          ],
        },
        {
          width: "100%",
          alignment: "center",
          text: "Facture " + billRef,
          bold: true,
          margin: [0, 10, 0, 10],
          fontSize: 15,
        },
        // ------ absolutePosition ------
        {
          absolutePosition: { x: 300, y: 10 },
          canvas: [
            {
              type: "rect",
              x: 1,
              y: 10,
              w: 175,
              h: 75,
              r: 4,
              lineColor: "#ccc",
              // color: '#ccc'
            },
          ],
        },
        // ------ absolutePosition ------
        {
          absolutePosition: { x: 310, y: 30 },
          text: [
            { text: "CLIENT: ", fontSize: 8 },
            { text: tob.client.username + "\n", bold: true, fontSize: 10 },
            { text: client + "\n" },
            { text: address + " " },
            { text: cp + "\n" },
            { text: city + "\n" },
          ],
          color: "#000",
          width: "auto",
          fontSize: 12,
          bold: true,
          alignment: "left",
          // margin: [100, 0, 0, 0],
        },
      ],

      content: [
        // ------ sale info ------
        {
          width: "50%",
          table: {
            headerRows: 1,

            // widths: [ 125, 125, 125, '*' ],
            widths: oWidth,
            heights: [6],
            // widths: [ '16%', '16%', '16%', '16%' ],
            body: [
              [
                { text: "COMMANDE N°", style: "headerText" },
                { text: "DATE", style: "headerText" },
                { text: " NBRE DE PDTS", style: "headerText" },
                { text: "QUANTITE TOTALE", style: "headerText" },
                { text: "NBRE COLIS", style: "headerText" },
                { text: "NBRE FROID", style: "headerText" },
              ],
              [
                { text: tob.ref, style: "rowText" },
                { text: dayjs(tob.saleDate).format("DD/MM/YYYY"), style: "rowText" },
                { text: nbProducts, style: "rowText" },
                { text: totalQtity, style: "rowText" },
                { text: nbColis, style: "rowText" },
                { text: nbFreeze, style: "rowText" },
              ],
            ],
          },
        },
        "\n",
        pdfTables,
        "\n\n",
        // ------ sum info ------
        infoAmount,
        // --------------------
        "\n\n",

        {
          text: [
            { text: nbrToLetterText, bold: true },
            // {text: nbrToLetter + ' francs CFA.', bold: true}
          ],
        },
      ],

      footer: [
        {
          // pageMargins: [40, 20, 40, 60],
          margin: [20, 40, 20, 0],
          columns: [
            // { canvas: [{ type: 'line', x1: 5, y1: 5, x2: 595 - 10, y2: 5, lineWidth: 0.2 }] },
            function (currentPage: { toString: () => string }, pageCount: string) {
              return currentPage.toString() + " of " + pageCount;
            },

            {
              text: [
                { text: "Domiciliation bancaire : ", alignment: "left", decoration: "underline" },
                "Orabank: TG0014 00216010000 88 - ",
                "UTB: TG009 01032 42515100400 62 - ",
                "Orabank: TG11601001030311892001 28 - \n",
                "Coris Bank: TG 182 01001 000075724101 12 - ",
                "NSIA Banque: TG 016 01001030311182001 28",
              ],
              color: "#333333",
              width: "auto",
              fontSize: 8,
              bold: true,
              // alignment: 'justify',
              alignment: "center",
            },
            {},
          ],
        },
      ],

      defaultStyle: {
        columnGap: 20,
      },
      styles: {
        textForm: {
          fillColor: "#eaf2f5",
          margin: [0, 5, 0, 5],
          textTransform: "uppercase",
          border: [false, true, false, true],
        },
        headerText: {
          // fillColor: '#255',
          // color: '#eee',
          border: [true, true, false, true],
          textTransform: "uppercase",
          fontSize: 8,
          bold: true,
        },
        rowText: {
          // fillColor: '#255',
          // color: '#eee',
          // border: [true, true, false, true],
          textTransform: "uppercase",
          fontSize: 10,
          margin: [0, 0, 5, 0],

          // bold: true
        },
      },
    };
  }
}

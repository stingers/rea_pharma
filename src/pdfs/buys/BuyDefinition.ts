import { Buy, labelSteName } from "asv-hlps";
import dayjs from "dayjs";
import { HlpUser } from "../../core/users/helpers/hlpUser";

export class BuyDefinition {
  static proforma(tob: Buy, pdfTables: any) {
    // const steGrpName = valToSingular(tob.pvd.ste.grp.name );
    const clientSte = tob.pvd.ste ? labelSteName(tob.pvd.ste, 30) : "";
    const clientLastname = tob.pvd.lastname ? tob.pvd.lastname : "";
    const clientFirstname = tob.pvd.firstname ? tob.pvd.firstname : "";
    const clientGender = tob.pvd.gender ? tob.pvd.gender.abr : "";
    const buyRef = "N° " + tob.ref;

    const waterMark = "Bon de commande";

    const client = tob.pvd.ste ? clientSte : clientGender + " " + clientLastname + " " + clientFirstname;
    const cp = tob.pvd.cp ? tob.pvd.cp : "";
    const city = tob.pvd.city ? tob.pvd.city : "";
    // const nif =  (tob.pvd.ste.nif) ? tob.pvd.ste.nif : '';
    const address = tob.pvd.address ? tob.pvd.address : "";
    const totalQtity = tob.totalQtity;
    const nbProducts = tob.nbProducts;
    ...
    return {
      ...
      header: [
        ...
        // ------ absolutePosition ------
        {
          ...
          text: [
            { text: "FOURNISSEUR: ", fontSize: 8 },
            { text: tob.pvd.username + "\n", bold: true, fontSize: 10 },
            { text: client + "\n" },
            { text: address + " " },
            { text: cp + "\n" },
            { text: city + "\n" },
      ...
      content: [
        ...
        /* {
          text: [
            {text: 'Arrêter cette facture à la somme de '},

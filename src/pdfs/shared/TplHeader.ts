import { Bill, BillPayment, User } from "asv-hlps";

import { HlpPdf } from "../hlpPdf";

export class TplHeader {
  static genericInvoice(client: User, bill: Bill) {
    // const clientSte =  (client.ste) ?  HlpUser.getSteShortname(client.ste) : '';
    /*  const clientLastname = (client.lastname) ? client.lastname : '';
    const clientFirstname = (client.firstname) ? client.firstname : '';
    const clientGender = (client.gender) ? client.gender.abr : ''; */
    let tobRef = "/BL";

    if (bill) {
      tobRef = bill.ref;
    }
    if (bill?.sales[0].isProforma) {
      tobRef = "/PROFORMA";
    }

    return this.getHeaderInvoice(client, tobRef, bill.store);

    /* const clientName = (client.ste) ? clientSte : clientGender + ' ' + clientLastname + ' ' + clientFirstname;
    const cp =  (client.cp) ? client.cp : '';
    const city =  (client.city) ? client.city : '';
    const nif =  (client.ste.nif) ? client.ste.nif : '';
    const address =  (client.address) ? client.address : ''; */
    // --------------------
    /* let logo, steName, address, qr
    switch (host) {
      case 'eqeer':
        logo = HlpPdf.disPlayLogoEqeer();
        address =  HlpPdf.displayAddressEqeer()
        qr = HlpPdf.displayQr('https://eqeer.in')
        steName= null
        break;
      case 'cpa':
        logo = HlpPdf.disPlayLogoCpa();
        steName = HlpPdf.displayCpaName()
        address =  HlpPdf.displayAddressCpa()
        qr = HlpPdf.displayQr('https://cpapharm.in')
        break;

      default: logo = HlpPdf.disPlayLogoCpa()
        break;
    }
    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {

              columns: [
                // HlpPdf.disPlayLogoCpa(),
                logo,

                // HlpPdf.displayCpaName(),
                steName

              ],

            },
            // HlpPdf.displayAddressCpa(),
            address

          ],
          // HlpPdf.displayQr('https://cpapharm.in'),
          qr

        ]
      },
      HlpPdf.displayRef(tobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      HlpPdf.displayRect(),

      // ------ absolutePosition ------
      HlpPdf.displayClient(client)
    ];


    return header; */
  }

  /* static generic(client: User, bill: Bill) {
    const clientSte =  (client.ste) ?  HlpUser.getSteShortname(client.ste) : '';
    const clientLastname = (client.lastname) ? client.lastname : '';
    const clientFirstname = (client.firstname) ? client.firstname : '';
    const clientGender = (client.gender) ? client.gender.abr : '';
    let tobRef = '/BL';

    if (bill) {
      tobRef =  bill.ref ;
    }
    if (bill?.sales[0].isProforma) {
      tobRef = '/PROFORMA';
    }

    const clientName = (client.ste) ? clientSte : clientGender + ' ' + clientLastname + ' ' + clientFirstname;
    const cp =  (client.cp) ? client.cp : '';
    const city =  (client.city) ? client.city : '';
    const nif =  (client.ste.nif) ? client.ste.nif : '';
    const address =  (client.address) ? client.address : '';
    // --------------------
    const cpaHeader = [
      {
        margin: [20, 20, 20, 0],
        columns: [

          [
            {

              columns: [
                {
                  image: 'data:image/png;base64,',
                  width: 40,
                  height: 40,
                  alignment: 'left',
                  // margin: [20, 20, 20, 20]


                },
                {

                  text: [
                    'COMPAGNIE \n ',
                    'PHARMACEUTIQUE \n ',
                    'AFRICAINE \n',

                  ],
                  width: 'auto',
                  alignment: 'left',
                  fontSize: 12,
                  bold: true,
                  margin: [-15, 0, 0, 0],

                },
              ],

            },
            {
              text: [
                {text: '01 BP 3701 514 AVENUE DES CALAIS\n', fontSize: 8},
                {text: 'Tél: (228) 22 22 27 71/ 22 22 34 74\n', fontSize: 8},
                {text: 'NIF: 10000175770 \n', bold: true, margin: [0, 0, 0, 2], fontSize: 8},
                // {text: 'fax: (228) 22 22 13 68 LOME - TOGO\n', fontSize: 8}
                {text: 'AUTORISATION N°33/97/MS/DGS/DPET \n', bold: true , fontSize: 10 },


              ]

            },

          ],

          {
            // qr: tob.bill.ref,
            qr: tobRef,
            alignment: 'right',
            fit: '50'
          },

        ]
      },
      {
        width: '100%',
        alignment: 'center',
        text: 'Facture  N° ' + tobRef,
        bold: true,
        margin: [0, 10, 0, 10],
        fontSize: 15,
      },
      // ------ absolutePosition ------
      {
        absolutePosition: { x: 300, y: 10},
        canvas: [
          {
            type: 'rect',
            x: 1,
            y: 10,
            w: 175,
            h: 75,
            r: 4,
            lineColor: '#ccc',
            // color: '#ccc'
          }
        ]
      },
      // ------ absolutePosition ------
      {

        absolutePosition: { x: 310, y: 30},
        text: [
          {text: 'CLIENT: ', fontSize: 8},
          {text: client.username + '\n', bold: true, fontSize: 10 },
          {text: clientName + '\n'},
          {text: address + ' ' },
          {text: cp + '\n'},
          {text: city + '\n'},
          // {text: nif + '\n'}

        ],
        color: '#000',
        width: 'auto',
        fontSize: 12,
        bold: true,
        alignment: 'left',
        // margin: [100, 0, 0, 0],
      },

    ];

    return cpaHeader;
  } */

  // static genericBl(client: User, tob: Bill, ) {
  // static genericBl(client: User, titleOrTobRef, host: 'eqeer'|'cpa' = 'cpa') {
  static genericInvoiceBl(client: User, host: string = "cpa") {
    const tobRef = "BORDEREAU DE LIVRAISON";

    return this.getHeaderInvoice(client, tobRef, host);
    /* let logo, steName, address, qr
    switch (host) {
      case 'eqeer':
        logo = HlpPdf.disPlayLogoEqeer();
        address =  HlpPdf.displayAddressEqeer()
        qr = HlpPdf.displayQr('https://eqeer.in')
        steName= null
        break;
      case 'cpa':
        logo = HlpPdf.disPlayLogoCpa();
        steName = HlpPdf.displayCpaName()
        address =  HlpPdf.displayAddressCpa()
        qr = HlpPdf.displayQr('https://cpapharm.in')
        break;

      default: logo = HlpPdf.disPlayLogoCpa()
        break;
    }


    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {

              columns: [
                // HlpPdf.disPlayLogoCpa(),
                logo,

                // HlpPdf.displayCpaName(),
                steName

              ],

            },
            // HlpPdf.displayAddressCpa(),
            address

          ],
          // HlpPdf.displayQr('https://cpapharm.in'),
          qr

        ]
      },
      HlpPdf.displayRef(titleOrTobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      HlpPdf.displayRect(),

      // ------ absolutePosition ------
      HlpPdf.displayClient(client)
    ];

    return header; */
  }

  static generic(titleOrTobRef: string, host: string) {
    return this.getHeader(titleOrTobRef, host);
  }

  static paymentReceipt(tob: BillPayment, host: string) {
    return this.getHeaderReceipt(tob, host);
  }

  private static getHeaderReceipt(tob: BillPayment, host: string = "cpa") {
    let logo, steName, address, qr;
    switch (host) {
      case "eqeer":
        logo = HlpPdf.disPlayLogoEqeer();
        address = HlpPdf.displayAddressEqeer();
        qr = HlpPdf.displayQr("https://eqeer.in");
        steName = null;
        break;
      case "cpa":
        logo = HlpPdf.disPlayLogoCpa();
        steName = HlpPdf.displayCpaName();
        address = HlpPdf.displayAddressCpa();
        qr = HlpPdf.displayQr("https://cpapharm.in");
        break;
    }

    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {
              columns: [
                // HlpPdf.disPlayLogoCpa(),
                logo,

                // HlpPdf.displayCpaName(),
                steName,
              ],
            },
            // HlpPdf.displayAddressCpa(),

            address,
          ],
          // HlpPdf.displayQr('https://cpapharm.in'),
          {
            alignment: "center",
            text: [{ text: `B.P.F :` + Math.abs(tob.paidAmount) + " FCFA", bold: true, frontSize: 10 }],
          },
          // qr
        ],
      },
      // HlpPdf.displayRef(titleOrTobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      // HlpPdf.displayRect(),

      // ------ absolutePosition ------
      // HlpPdf.displayClient(client)
    ];

    return header;
  }

  private static getHeader(titleOrTobRef: string, host: string = "cpa") {
    let logo, steName, address, qr;
    switch (host) {
      case "eqeer":
        logo = HlpPdf.disPlayLogoEqeer();
        address = HlpPdf.displayAddressEqeer();
        qr = HlpPdf.displayQr("https://eqeer.in");
        steName = null;
        break;
      case "cpa":
        logo = HlpPdf.disPlayLogoCpa();
        steName = HlpPdf.displayCpaName();
        address = HlpPdf.displayAddressCpa();
        qr = HlpPdf.displayQr("https://cpapharm.in");
        break;

      default:
        logo = HlpPdf.disPlayLogoCpa();
        break;
    }

    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {
              columns: [
                // HlpPdf.disPlayLogoCpa(),
                logo,

                // HlpPdf.displayCpaName(),
                steName,
              ],
            },
            // HlpPdf.displayAddressCpa(),
            address,
          ],
          // HlpPdf.displayQr('https://cpapharm.in'),
          qr,
        ],
      },
      HlpPdf.displayRef(titleOrTobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      // HlpPdf.displayRect(),

      // ------ absolutePosition ------
      // HlpPdf.displayClient(client)
    ];

    return header;
  }

  private static getHeaderInvoice(client: User, titleOrTobRef, host: string = "cpa") {
    let logo, steName, address, qr;
    switch (host) {
      case "eqeer":
        logo = HlpPdf.disPlayLogoEqeer();
        address = HlpPdf.displayAddressEqeer();
        qr = HlpPdf.displayQr("https://eqeer.in");
        steName = null;
        break;
      case "cpa":
        logo = HlpPdf.disPlayLogoCpa();
        steName = HlpPdf.displayCpaName();
        address = HlpPdf.displayAddressCpa();
        qr = HlpPdf.displayQr("https://cpapharm.in");
        break;

      default:
        logo = HlpPdf.disPlayLogoCpa();
        break;
    }

    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {
              columns: [
                // HlpPdf.disPlayLogoCpa(),
                logo,

                // HlpPdf.displayCpaName(),
                steName,
              ],
            },
            // HlpPdf.displayAddressCpa(),
            address,
          ],
          // HlpPdf.displayQr('https://cpapharm.in'),
          qr,
        ],
      },
      HlpPdf.displayRef(titleOrTobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      HlpPdf.displayRect(),

      // ------ absolutePosition ------
      HlpPdf.displayClient(client),
    ];

    return header;
  }

  /* static genericBl(client: User, titleOrTobRef, host: string = 'cpa') {

    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {

              columns: [
                HlpPdf.disPlayLogoCpa(),

                HlpPdf.displayCpaName()

              ],

            },
            HlpPdf.displayAddressCpa(),

          ],
          HlpPdf.displayQr('https://cpapharm.in'),

        ]
      },
      HlpPdf.displayRef(titleOrTobRef),
      // HlpPdf.displayTitle(tobRef),

      // ------ absolutePosition ------
      HlpPdf.displayRect(),

      // ------ absolutePosition ------
      HlpPdf.displayClient(client)
    ];

    return header;
  } */

  /*  static genericBlEqeer(client: User, titleOrTobRef, host: 'cpa'|'eqeer' = 'cpa') {
    let logo
    switch (host) {
      case 'eqeer':
        logo = HlpPdf.disPlayLogoEqeer();
        break;
      case 'cpa':
        logo = HlpPdf.disPlayLogoCpa();
        break;

      default: logo = HlpPdf.disPlayLogoCpa()
        break;
    }

    // --------------------
    const header = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          [
            {

              columns: [

                logo
                // HlpPdf.disPlayLogoEqeer(),

                // HlpPdf.displayCpaName()

              ],

            },
            HlpPdf.displayAddressEqeer(),

          ],
          HlpPdf.displayQr('https://eqeer.in'),

        ]
      },
      // HlpPdf.displayRef(tobRef),
      HlpPdf.displayRefEqeer(titleOrTobRef),

      // ------ absolutePosition ------
      HlpPdf.displayRect(),

      // ------ absolutePosition ------
      HlpPdf.displayClient(client)
    ];

    return header;
  } */
}

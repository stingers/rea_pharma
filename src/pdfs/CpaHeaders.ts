import { User, Bill, getSteShortname } from "asv-hlps";
import { HlpUser } from "../core/users/helpers/hlpUser";
import { HlpPdf } from "./hlpPdf";

export class CpaHeaders {
  static invoice(client: User, tob: any) {
    ...
    const cpaHeader = [
      {
        margin: [20, 20, 20, 0],
        columns: [
          HlpPdf.displayCpaHead(),

          [
            {
              columns: [HlpPdf.disPlayLogoCpa(), HlpPdf.displayCpaName()],
            },
            HlpPdf.displayAddressCpa(),
          ],
          HlpPdf.displayQr(tobRef),
  ...
  static generic(title?: string, tob?: any) {
    const hTitle = title ? HlpPdf.displayTitle(title) : null;
    const cpaHeader = [
      {
        margin: [20, 20, 20, 0],
        columns: [HlpPdf.displayCpaHead(), HlpPdf.displayQr("http://cpapharm.in")],
      },
      hTitle,
      // HlpPdf.displayTitle(title),
  ...
  static genericBl(client: User, tob: Bill) {
    ...
    const clientGender = client.gender ? client.gender.abr : "";
    // const billRef =   'N° ' + tob.ref ;
    let tobRef = "BORDEREAU DE LIVRAISON";
  ...
  static genericTitleArray(title: string[], tob?: any) {
    const title1 = title[0].toUpperCase();
    const title2 = title[1].toUpperCase();
    // --------------------
    const cpaHeader = [
      {
        margin: [20, 20, 20, 0],
...

      {
        alignment: "center",
        text: [
          { text: title1 + "\n", bold: true, margin: [0, 10, 0, 10], fontSize: 15 },

          { text: title2, bold: true, margin: [0, 10, 0, 10], fontSize: 15 },
        ],
        // width: '100%',
  ...
  static buy(title: any, tob?: any) {
    ...
    const cpaHeader = [
      ...
      },
      {
        columns: [
          {
            text: "Commande N°" + tob.ref + " du " + tob.createdAt,
            alignment: "center",
            fontSize: 20,
          },
          "\n",
